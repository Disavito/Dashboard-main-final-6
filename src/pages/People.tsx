...
function People() {
  ...
  const fetchSocios = useCallback(async () => {
    setLoading(true);
    try {
      // Parallelizar ambas consultas y seleccionar SOLO columnas necesarias
      const [sociosRes, ingresosRes] = await Promise.all([
        supabase
          .from('socio_titulares')
          .select('id, dni, nombres, apellidoPaterno, apellidoMaterno, celular, localidad, mz, lote, is_lote_medido, isObservado, observacion, is_payment_observed, payment_observation_detail')
          .order('apellidoPaterno', { ascending: true }),
        supabase
          .from('ingresos')
          .select('dni, receipt_number, transaction_type, amount, date')
          .neq('dni', null)
      ...
      const enrichedSocios: EnrichedSocio[] = (sociosData || []).map(socio => {
        ...
        if (lastTransaction) {
          ...
          if (type.includes('anulacion') || type.includes('devolucion') || amount < 0) status = 'Retirado';
  ...
  }, [mobileVisibleCount, socios, searchInput, selectedLocalidad, selectedEstado, filteredData.length]);
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] pb-10">
      ...
      <Dialog open={isExportDialogOpen} onOpenChange={setIsExportDialogOpen}>
        <DialogContent className="max-w-3xl bg-white border-none rounded-2xl p-0 overflow-hidden">
